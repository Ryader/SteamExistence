...
public class Drag : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
{
    ...
    void Start()
    {
        characterInventory = GameObject.FindGameObjectWithTag("Player").transform.GetComponent<CharacterInventory>();
        if (typeList == "")
            typeList = "Ground";
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        if (eventData.button == PointerEventData.InputButton.Right)
        {
            ...
            {
                characterInventory.UseItem(this);
            }
            else if(typeList == "Ground" && item.typeItem == "Other")
            {
                characterInventory.TakeItem(this);
            }
            else if(typeList == "Ground" && item.typeItem == "First Weapon")
            {
                characterInventory.TakeWeapon(this);
            }
        }
